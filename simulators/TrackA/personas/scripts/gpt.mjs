...
function loadEnvKey() {
  ...
  // Look in personas/.env first, then the workspace-root .env one level up.
  return readKeyFromEnvFile(join(ROOT, ".env")) || readKeyFromEnvFile(join(ROOT, "..", ".env"));
}
...
/**
 * Single-shot JSON decision. Forces response_format=json_object and returns the
 * parsed object. Used by the page-by-page persona funnel agent.
 */
export async function decideJSON({ system, user, model = MODEL, temperature = 0.8 }) {
  if (!KEY) throw new Error("No OPENAI_API_KEY found (.env or env).");

  // Measure the REAL wall-clock time this reasoning call takes. This is the
  // persona's genuine "thinking time" for the screen — not an estimate. Token
  // usage (esp. reasoning tokens) is a second, real measure of deliberation depth.
  ...
  try {
    return { model, _latencyMs: latencyMs, _usage: data.usage || null, ...JSON.parse(text) };
  } catch {
