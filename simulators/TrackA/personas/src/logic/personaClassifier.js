...
import { DWELL_THRESHOLDS, msToReadable } from "../logs/constants.js";
...
export function extractSignals(snap) {
  ...
  const completed =
    events.some((e) => e.type === "funnel_complete") || snap.currentStep === 8;
  const abandonedAtStep = abandons.length ? abandons[abandons.length - 1].step : null;
  const reachedFinal = !!pages[8] || (pages[6] && completed);
  const route = snap.summary?.projectedRoute;
