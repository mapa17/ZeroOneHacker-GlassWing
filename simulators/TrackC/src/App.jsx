...
import { callClaude, openaiChat, safeParse, wait } from "./agent/api.js";
import { createCoachSession, COACH_SYSTEM_PROMPT, buildObservation, buildUserQuestion } from "./agent/coach.js";
...
// Advisor-warning coach is opt-in via the ?enable-coach URL flag (mirrors the CLI
// agent's --enable-coach). When absent, the coach is fully disabled.
const COACH_ENABLED =
  typeof window !== "undefined" &&
  new URLSearchParams(window.location.search).has("enable-coach");
...
export default function App() {
  ...
  // LLM coach session — persists for the lifetime of the app instance.
  const coachSessionRef = useRef(null);
  if (COACH_ENABLED && !coachSessionRef.current) {
    coachSessionRef.current = createCoachSession({
      chatFn: openaiChat,
  ...
  // Call the coach LLM after each step advance (page change).
  useEffect(() => {
    if (!COACH_ENABLED || step === 8) return;
    const session = coachSessionRef.current;
    if (!session) return;
    setCoachLoading(true);
    const observation = buildObservation(step, null, form);
    session.observe(observation).then((response) => {
      setCoachLoading(false);
      if (response?.message) {
        setCoachMessages((m) => [...m, { role: "coach", text: response.message, step, coachResponse: response }]);
      }
  ...
  const sendCoachMessage = async (text) => {
    const t = (text || "").trim();
    if (!t || !COACH_ENABLED) return;
    const currentStep = step;
    setCoachMessages((m) => [...m, { role: "user", text: t, step: currentStep }]);
    setCoachLoading(true);
    const session = coachSessionRef.current;
    if (!session) { setCoachLoading(false); return; }
    const questionText = buildUserQuestion(t, currentStep, form);
    const response = await session.reply(questionText).catch(() => null);
    setCoachLoading(false);
    if (response?.message) {
      setCoachMessages((m) => [...m, { role: "coach", text: response.message, step: currentStep, coachResponse: response }]);
    }
  };
  ...
  return (
    <div style={{ background: C.lab, minHeight: "100vh", padding: "18px", fontFamily: "Outfit, sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,420px)", gap: 16, alignItems: "start" }}
        ...
        <div>
          {COACH_ENABLED && (
            <CoachChat messages={coachMessages} loading={coachLoading} onSend={sendCoachMessage} />
          )}
