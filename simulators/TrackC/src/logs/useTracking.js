...
import { terminalStatus } from "../logic/form.js";
import {
...
export function useTracking({ step, form, mode, outcome, premium, decorate }) {
  ...
  useEffect(() => {
    ...
    trackEvent("funnel_complete", {
      route: outcome.route,
      // "completed" only for the online path (incl. step-7 Online Videoberatung);
      // advisor-routed sessions reaching step 8 are "advisor_forward", not conversions.
      status: terminalStatus(form),
      tarif: form.tarif,
  ...
  const buildSnapshot = useCallback(() => {
    ...
    // Reaching the result page (step 8) is not the same as completing: classify it.
    const reachedOutcome = curStep === 8;
    const status = reachedOutcome ? terminalStatus(form) : null;
