...
function resolveSessionsDir() {
  ...
  if (!existsSync(base)) throw new Error("No test-results/ folder. Run npm run profiles:sample first.");
  const runs = readdirSync(base).filter((d) => existsSync(join(base, d, "sessions"))).sort();
  if (!runs.length) throw new Error("No runs with a sessions/ folder found.");
  return join(base, runs[runs.length - 1], "sessions");
}
