...
/** Parse .env and populate process.env for any key not already set in the environment. */
...
export const getOpenAIModel = () => process.env.CUSTOMER_MODEL || process.env.OPENAI_MODEL || "gpt-4o-mini";
export const getCoachModel  = () => process.env.COACH_MODEL || "gpt-4o-mini";
...
export async function chatCompletion({ system, messages, temperature = 0.7, model } = {}) {
  ...
  const res = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({ model: model || MODEL, temperature, messages: apiMessages }),
  });
  if (!res.ok) {
