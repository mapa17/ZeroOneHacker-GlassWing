...
import { describeCoachForStep } from "./coachState.js";
...
export function describeStep({ step, form, coachState, premium, profile, validationErrors, canProceed, focusedMode = false }) {
  const coachBlock = describeCoachForStep(coachState, step);
...
## Your task
Decide what to do on THIS step only.
Reply with a single JSON object, no markdown or prose.
Describe any reasoning and decision in German.
Always decise based on the profile if you want to leave the funnel or not. If in doubt, leave the funnel.

Allowed action for this step:
${actionHintForStep(step, { focusedMode })}
${exitBehaviorRulesBlock()}`;
}
